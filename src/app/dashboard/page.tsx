...
export default function DashboardPage() {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      <div>
        <h1 className="text-2xl font-extrabold">مرحباً {profile.name} 👋</h1>
        <p className="text-muted-foreground">هذه نظرة سريعة على نشاطك</p>
      </div>
        ...
        <Card className="lg:col-span-1">
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center gap-2">
              <code dir="ltr" className="min-w-0 flex-1 truncate rounded-lg bg-muted px-3 py-2 text-xs">
                {pageUrl}
              </code>
              <Button variant="outline" size="icon" onClick={copyLink} aria-label="نسخ الرابط">
                ...
                <ul className="divide-y">
                  {invoices.map((invoice) => (
                    <li key={invoice.id} className="flex items-center justify-between gap-3 py-3">
                      <div className="min-w-0">
                        <div className="truncate font-semibold">{invoice.clientName}</div>
